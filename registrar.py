...
class FixedDataRegistrar:
    ...
    def get_fixed_data(self, object_type: Nameable, item_reference: any):
    ...
    def create_player(self, name: str):
        ...
        self._store[Nameable.PLAYER].append(new_player)
        self._id_counter += 1
        return new_player

    def create_team(self, name: str, line_up: list[Player]):
        new_team = Team(self._id_counter, name, line_up)
        self._store[Nameable.TEAM].append(new_team)
        self._id_counter += 1
        return new_team

    def get_all_of_type(self, fd_type: Nameable):
        return self._store[fd_type]

    def get_from_names(self, fd_type: Nameable, names: list[str]):
        fixed_data_items = []
        for name in names:
